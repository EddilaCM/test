...
public class basedUserCFS {
	...
	public static String getRecommend(Map<String, Map<String, Integer>> simUserObjMap, Map<String, Double> simUserSimMap) {
		Map<String, Double> objMapScoreMap = new HashMap<String, Double>();
		for (Entry<String, Map<String, Integer>> simUserEn: simUserObjMap.entrySet()) {
			...
			for (Entry<String, Integer> simObjEn : simUserEn.getValue().entrySet()) {
				...
				if (objMapScoreMap.get(objName) == null) {
					objMapScoreMap.put(objName, objScore);
				}else {
					double totalScore = objMapScoreMap.get(objName);
					objMapScoreMap.put(objName, totalScore + objScore);							
				}
			}
		}
		List<Entry<String, Double>> enList = new ArrayList<Map.Entry<String,Double>>();
